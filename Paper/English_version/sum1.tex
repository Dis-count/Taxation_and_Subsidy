...
Notice that we can calculate the characteristic function $c(s)$ easily according to Theorem \ref{thm5}, we can formulate Theorem \ref{thm6}. Until here, we've solved the AIPU game with alternative machines.
...
Now we know the separation problem can be solved in $O(v^2)$ time and the breakpoints during the whole interval are polynomial, so for the AIPU game the PSPF function can be constructed in polynomial time.

\begin{thm}\label{thm6}
The AIPU game can be solved in polynomial time.
\end{thm}
