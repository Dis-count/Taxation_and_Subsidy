...
When the setup cost increases from 9.5 to 10, that is, the taxation increases from 0 to 0.5, the number of machine will decrease from 2 to 1, accordingly.

When the setup cost increases from 9.5 to 11.14, that is, the price increment is from 0 to 1.64, for the grand coalition, it only needs one machine.

In this case, the total pricing can exactly cover the gap, which means the grand coalition can be stabilized by the players themselves.
...
Then we need to define an interval $[P_L(i,s),P_H(i,s)]$ to denote the price range of using $i$ machines for scheduling jobs among players $s$. And the effective domain of pricing MSG for stabilization is $[0,P^*]$.
...
\begin{remark}
  $P^*$ is the lowest price under which the grand cooperation of MSG is stable and it uses only one machine in the optimal scheduling decision, i.e.,
