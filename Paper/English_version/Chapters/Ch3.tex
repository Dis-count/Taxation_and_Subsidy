...
When the setup cost increases from 9.5 to 10, that is, the price increment is from 0 to 0.5, the number of machine used by the grand coaliton will decrease from 2 to 1, accordingly. However, at this time, the grand coalition still needs extra subsidy in addition to the price increment.

Continue to increase the setup cost from 10 to 11.14, , for the grand coalition, one machine is still used. At this point, the total pricing increment can exactly cover the gap of total cost $c(V)$ and the acccepted cost shared among players in the grand coalition $\alpha(V)$, which means that by using pricing instrument the grand coalition can be stabilized by the players themselves.
